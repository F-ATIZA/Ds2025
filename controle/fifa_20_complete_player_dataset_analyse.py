# -*- coding: utf-8 -*-
"""FIFA 20 complete player dataset analyse.ipynb
...
### Subtask:
Optimize the hyperparameters for each of the selected machine learning models using advanced search techniques like GridSearchCV or RandomizedSearchCV. This aims to find the best configuration for each model to maximize performance on the task.

**Reasoning**:
As instructed, I will first import the necessary modules, define parameter grids for each model, and create a custom RMSE scorer to prepare for hyperparameter optimization.
"""
...
param_grid_rfr = {
    ...
    'max_features': [0.6, 0.8, 1.0], # Using floats for max_features for clarity in RandomForest
...
# 2. Create a custom RMSE scorer
# GridSearchCV optimizes for 'greater_is_better', so we use negative MSE
rmse_scorer = make_scorer(mean_squared_error, greater_is_better=False, squared=False)

print("Parameter grids defined and custom RMSE scorer created.")

"""**Reasoning**:
Now that the parameter grids and a custom RMSE scorer are defined, I will iterate through each model, initialize `GridSearchCV` with the appropriate parameters, fit it to the training data, and then print the best hyperparameters and the corresponding RMSE score.
...
for name, model in models.items():
    ...
    grid_search = GridSearchCV(
        ...
        cv=kf,
        scoring=rmse_scorer, # Using the custom RMSE scorer
    ...
    best_rmse = -grid_search.best_score_ # best_score_ is negative RMSE, so negate it
